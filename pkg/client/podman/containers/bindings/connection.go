...
import (
	...
	"github.com/silenium-dev/docker-wrapper/pkg/client/podman/containers/ssh"
...
var connectionFactories = map[string]func(_url *url.URL, uri string, identity string, machine bool) (Connection, error){
	"ssh":  sshClient,
	"unix": unixClient,
	"tcp":  tcpClient,
}
...
func NewConnectionWithIdentity(ctx context.Context, uri string, identity string, machine bool) (context.Context, error) {
	...
	if factory, found := connectionFactories[_url.Scheme]; found {
		connection, err = factory(_url, uri, identity, machine)
		if err != nil {
			return nil, err
		}
	} else {
...
func tcpClient(_url *url.URL, uri string, _ string, _ bool) (Connection, error) {
	connection := Connection{
		URI: _url,
	}
	if !strings.HasPrefix(uri, "tcp://") {
		return connection, errors.New("tcp URIs should begin with tcp://")
	}
	dialContext := func(ctx context.Context, _, _ string) (net.Conn, error) {
	...
	if proxyURI, found := os.LookupEnv("CONTAINER_PROXY"); found {
		...
		if err != nil {
			return connection, newConnectError(fmt.Errorf("unable to dial to proxy %s, %w", proxyURI, err))
		}
		dialContext = func(ctx context.Context, _, _ string) (net.Conn, error) {
...
func unixClient(_url *url.URL, uri string, _ string, _ bool) (Connection, error) {
